...
for zz = 1:length(trellis)
%% Encodeur
encodeur=comm.ConvolutionalEncoder('TrellisStructure',trellis(zz),'TerminationMethod','Terminated');
%% Decodeur
decodeur = comm.ViterbiDecoder('TrellisStructure',trellis(zz),'TracebackDepth',backdepth,'TerminationMethod','Terminated','InputFormat','Unquantized');
%% Simulation
for i_snr = 1:length(EbN0dB)
    ...
    while (err_stat(2) < nbr_erreur && err_stat(3) < nbr_bit_max)
        ...
        encode = encodeur(msg);       % Encodage
        x      = step(mod_psk,  encode); % Modulation QPSK
        ...
        decode = decodeur(Lc);         % Decodage
        %rec_msg = double(decode(1:K) < 0); % Décision
        rec_msg = decode(1:length(msg));
