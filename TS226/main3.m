...
trellis1 = poly2trellis(2,[2 3],2);
trellis2 = poly2trellis(3,[7 5],7);
trellis3 = poly2trellis(4,[13 15],13);
trellis4 = poly2trellis(7,[133 171],133);

%trellis = vertcat(trellis1,trellis2,trellis3,trellis4);
trellis = vertcat(trellis4)
constLen = 2 ; 
backdepth = 5 * constLen;
%% Encodage de Viterbi 
...
%% Vitterbi Encodeur
vit_encodeur = comm.ConvolutionalEncoder('TrellisStructure',trellis4,'TerminationMethod','Terminated');
%% Vitterbi Decodeur
vit_decodeur = comm.ViterbiDecoder('TrellisStructure',trellis4,'TracebackDepth',backdepth,'TerminationMethod','Terminated','InputFormat','Unquantized');
%% RS Encoder
rs_encoder = comm.RSEncoder(63,55,'BitInput',true);
%% RS Decoder
rs_decoder = comm.RSDecoder(63,55,'BitInput',true);
%% Entrelaceur
entrelaceur = comm.MatrixInterleaver(378,100);
%% Desentrelaceur 
desentrelaceur = comm.MatrixDeinterleaver(378,100);
%% Simulation
for i_snr = 1:length(EbN0dB)
    ...
    while (err_stat(2) < nbr_erreur && err_stat(3) < nbr_bit_max)
        ...
        rs_encode = rs_encoder(msg);     %Reed_Solomon encodage
        entrelacer = entrelaceur(rs_encode);
        vit_encode = vit_encodeur(entrelacer);       % Encodage
        x      = step(mod_psk,  vit_encode); % Modulation QPSK
        ...
        vit_decode = vit_decodeur(Lc);         % Decodage
        desentrelacer = desentrelaceur(vit_decode(1:length(rs_encode)));
        %rs_decode = rs_decoder(vit_decode(1:length(rs_encode)));
        rs_decode = rs_decoder(desentrelacer);
        
        %rec_msg = double(decode(1:K) < 0); % Décision
        rec_msg = rs_decode(1:length(msg));
        %rec_msg = Lc(1:length(msg));
...
xlabel('$\frac{E_b}{N_0}$ en dB','Interpreter', 'latex', 'FontSize',14)
ylabel('TEB','Interpreter', 'latex', 'FontSize',14)
%legend({'TEB cod\''e (133,171)'}, 'Interpreter', 'latex', 'FontSize',14);
%save('TEB1.mat','EbN0dB','ber')
%ber = zeros(1,length(EbN0dB));


%rd fermé !! ne pas oublier le parmetre k en plus de la memoire : 2*(k+m)